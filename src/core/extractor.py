...
class ExtractionState(TypedDict):
    ...
    queries: list
    final_url: list
    awaiting_user_action: bool = False  # New field for web interface
...
class CoreConceptExtractor:
    ...
    def step2_keyword_generation(self, state: ExtractionState) -> ExtractionState:
        ...
        # Set flag for web interface to wait for user action
        return {
    ...
    def step3_human_evaluation(self, state: ExtractionState) -> ExtractionState:
        ...
        # Check if we're awaiting user action from web interface
        if state.get("awaiting_user_action", False):
            # Return state as is, web interface will handle the validation
            return state
            
        feedback = state.get("validation_feedback")
        if not feedback:
            # If no feedback is provided, wait for it
            return {
                "awaiting_user_action": True,
                "concept_matrix": state["concept_matrix"],
        ...
        # Clear the awaiting flag since we have feedback
        state["awaiting_user_action"] = False
